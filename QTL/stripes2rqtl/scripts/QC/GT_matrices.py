...
class Genotypes(object):
    ...
    def add_genotypes(self, gt_file):
        ...
        if self.subset:
            print("you're appending new genotypes to a genotype matrix that has previously been filtered. the filtering criteria are stored in self.subset ")
    ...
    def fill_in_parental_congruency(self):
        ...
        for sample in self.samples:
            ...
            if self.congruency_matrix.loc[sample].isnull().sum().sum() == len(self.congruency_matrix.loc[sample]):
                if not self.pedigree.get_offspring(sample=sample) == []:
    ...
    def identify_bad_parents(self, generation, cutoff_fraction=0.8, quality_threshold=0.55):
        """
    ...
    def _filter_by_parentage(self,generation,cutoff_fraction=0.8,quality_threshold=0.55):
        """ filter a specific generation by the fraction of good offspring they have. """
        mask = pd.DataFrame(np.zeros_like(self.gt_matrix))
        bp, bpo = self.identify_bad_parents(generation=generation,cutoff_fraction=cutoff_fraction,quality_threshold=quality_threshold)
    ...
    def filter_by_parentage(self, cutoff_fraction=0.8, quality_threshold=0.55):
        """ filter all individuals by the fraction of good offspring. """
        generations_present = set([ i[-2::] for i in test.samples])
        for generation in generations_present: # for genotyped generations
            #print("0"+str(int(generation)))
            if "0"+str(int(generation)+1) in generations_present: # check if offspring is also genotyped
                mask =  self._filter_by_parentage(generation=generation, cutoff_fraction=cutoff_fraction, quality_threshold=0.55)
    ...
    def smooth_small_xo(self, array, thresh=3):
    ...
    def _xo_filter(self,genotype_matrix, thresh=3):
        """filter genotype matrix """
        filter_matrix = []
        for i, k in genotype_matrix.iterrows():
            filter_matrix.append(self.smooth_small_xo(array=k, thresh=thresh))
