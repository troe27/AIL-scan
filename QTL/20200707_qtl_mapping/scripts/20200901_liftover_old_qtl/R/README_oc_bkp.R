...
source("./R/Functions.AIL.impute.R")
library(data.table)
...
# Check coverage of F2 individuals
# coverage1 is the mean coverage. ie.if the site A has two reads will be counted as 2. 
# coverage2 isthe number of sites with at least a read. 
cov <- file("./data/F2.all.coverage1212.txt")
open(cov)
num.line <-  unlist(strsplit(system("wc -l ./data/F2.all.coverage1212.txt",intern = T),split = " "))[5]
...
#And output to file
coverage <- as.matrix(coverage2)
rownames(coverage) <- id3
write.table(coverage, file = "./data/180208_F2_coverage.txt", row.names = TRUE)
...
res <- hist(pos.chr1,ceiling(max(pos.chr1)/bin.size))
res$counts
